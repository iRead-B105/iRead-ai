...
class GeneratedStoryChapterPagePayload(StoryPageContractModel):
    ...
    choices: list[NonEmptyText] = Field(max_length=2)
    requires_branch_input: StrictBool

    @model_validator(mode="after")
    def validate_branch_contract(self) -> Self:
        if self.requires_branch_input:
            if self.question is None or len(self.choices) != 2:
                raise ValueError(
                    "a branching page requires one question and exactly two choices"
                )
