...
def build_chapter_request(
    ...
    if chapter_index != expected_index:
        raise DynamicStoryStateError(
            "요청 장과 마지막으로 저장한 이야기 장이 이어지지 않습니다."
        )
    ...
    if chapter_index == 0:
        if branch_input is not None:
            raise DynamicStoryStateError(
                "첫 장에는 이전 질문의 아이 답을 전달할 수 없습니다."
            )
    else:
        if branch_input is None:
            raise DynamicStoryStateError(
                "다음 장을 생성하려면 직전 질문의 아이 답이 필요합니다."
            )
    ...
    payload: dict[str, Any] = {
        ...
        "chapterPlan": {
            "orderedEvents": [
                {
                    ...
                    "requiredCharacters": list(
                        page.required_character_ids
                    ),
            ...
            "maxPages": 4,
            "questionFocus": (
                None if beat.concluding else beat.question_focus
            ),
        },
        "branchInput": (
...
def apply_chapter_response(
    ...
    applied_ids = [
        str(value)
        for value in runtime.get("appliedGenerationIds", [])
    ]
    ...
    if response.story_revision != current_revision:
        raise DynamicStoryStateError(
            "응답 storyRevision이 현재 이야기 revision과 다릅니다."
        )
    if response.state_patch.expected_base_revision != current_revision:
        raise DynamicStoryStateError(
            "응답의 expectedBaseRevision이 현재 revision과 다릅니다."
        )
    if response.chapter_number != current_chapter + 1:
        raise DynamicStoryStateError(
            "응답 장 번호가 마지막으로 저장한 장과 이어지지 않습니다."
        )
    if response.generation.policy_hash != str(
        _generation_profile(runtime).get("policyHash")
    ):
        raise DynamicStoryStateError(
            "응답 policyHash가 요청에 사용한 프로필과 다릅니다."
        )
    if response.generation.generation_profile_version != int(
        ...
        raise DynamicStoryStateError(
            "응답 generationProfileVersion이 현재 프로필과 다릅니다."
        )
    ...
    if final_page.requires_branch_input == is_final_chapter:
        raise DynamicStoryStateError(
            "마지막 장 여부와 최종 페이지의 분기 상태가 맞지 않습니다."
        )
    if response.state_patch.last_question != final_page.question:
        raise DynamicStoryStateError(
            "statePatch.lastQuestion이 마지막 페이지 질문과 다릅니다."
        )
    ...
    remaining_hooks = [
        hook
        for hook in story_state.get("unresolvedHooks", [])
        if hook not in removed_hooks
    ]
...
def displayed_page(
    ...
    if response.chapter_number != cursor.chapter_number:
        raise DynamicStoryStateError(
            "화면 커서의 장 번호가 응답과 다릅니다."
        )
    if len(response.pages) != cursor.page_count:
        raise DynamicStoryStateError(
            "화면 커서의 pageCount가 응답과 다릅니다."
        )
...
def _generation_profile(runtime: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(value, Mapping):
        raise DynamicStoryStateError(
            "runtime.generationProfile이 없습니다."
        )
    return {str(key): item for key, item in value.items()}
