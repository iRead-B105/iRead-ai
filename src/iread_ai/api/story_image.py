...
from iread_ai.application.story_image_service import StoryImageUseCaseError
...
router = APIRouter(prefix="/api/v1/story/images", tags=["story-images-v1"])
...
ERROR_RESPONSES = {
    ...
    422: {"model": ErrorResponse, "description": "장면 또는 레퍼런스 검증 실패"},
    502: {"model": ErrorResponse, "description": "이미지 모델 응답 오류"},
    503: {"model": ErrorResponse, "description": "이미지 공급자 또는 에셋 사용 불가"},
    504: {"model": ErrorResponse, "description": "이미지 모델 처리 시간 초과"},
}
...
@router.post(
    "/generate",
    response_model=StoryImageGenerateResponse,
    responses=ERROR_RESPONSES,
    operation_id="generateStoryPageImage",
    summary="이야기 문장과 visualScene으로 페이지 삽화 생성",
    response_description="캔버스 전체를 사용하는 풀블리드 21:9 페이지 삽화",
    openapi_extra=OPENAPI_OPERATION_POLICY,
)
async def generate_story_page_image(
    payload: StoryImageGenerateRequest,
    request: Request,
    auth: Auth,
    idempotency_key: IdempotencyKey,
) -> StoryImageGenerateResponse | JSONResponse:
    ...
    if begin.kind is BeginKind.CONFLICT:
        raise ApiError(
            ...
            message="같은 Idempotency-Key에 다른 요청 본문이 사용되었습니다.",
    ...
    if begin.kind is BeginKind.IN_PROGRESS:
        raise ApiError(
            ...
            message="같은 이미지 생성 요청이 이미 처리 중입니다.",
            request_id=payload.request_id,
            retryable=True,
        )
    ...
    try:
        response = await request.app.state.story_image_service.generate(payload)
        ...
        return response
    except StoryImageUseCaseError as exc:
        api_error = ApiError(
...
def _fingerprint(payload: StoryImageGenerateRequest) -> str:
