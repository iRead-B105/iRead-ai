...
class ChapterGenerationComparisonService:
    ...
    async def compare_displayed_chapter_to_plain(
        ...
        if baseline_response.generation.api_call_count != 1:
            raise RuntimeError("baseline chapter comparison must make exactly one model call")
        if baseline_response.generation.repair_attempted:
            raise RuntimeError("baseline chapter comparison must not attempt repair")
        ...
        return {
            ...
            "diagnostics": {
                "baselineApiCallCount": (baseline_response.generation.api_call_count),
                "personalizedApiCallCount": (personalized_response.generation.api_call_count),
                "newApiCallCount": 1,
                "baselineRepairAttempted": (baseline_response.generation.repair_attempted),
                "comparisonWallElapsedMs": round(wall_ms, 3),
                "execution": ("one_new_plain_chapter_call_after_personalized_chapter"),
            },
        }
...
def _outcome(
    ...
    *,
    ...
    return {
        "pipeline": pipeline,
        "status": "SUCCESS",
        "pages": [page.model_dump(mode="json", by_alias=True) for page in response.pages],
        "fit": fit,
...
def _profile_fit(
    ...
    quality_by_code = {skill.code: skill for skill in quality.per_skill}
    unverified_skill_codes = [
        policy.code
        for policy in request.generation_profile.skills
        if (policy.code.startswith("PHONO_") and quality.analysis_status != "FULL")
        or (policy.code in quality_by_code and quality_by_code[policy.code].status == "UNVERIFIED")
    ]
    ...
    if score_confidence == "PARTIAL":
        ...
        score_reason = (
            "동일한 아동 프로필로 Kiwi·G2P 분석 후 100×exp(-0.55×riskPer10)을 계산했습니다."
        )
    per_skill = [skill.model_dump(mode="json", by_alias=True) for skill in quality.per_skill]
    return {
        "status": quality.status,
        "comparable": True,
        "analysisStatus": quality.analysis_status,
        "scoreConfidence": score_confidence,
        "scoreBasis": ("SURFACE_ONLY" if score_confidence == "PARTIAL" else "FULL_POLICY"),
        ...
        "targetDistance": sum(int(skill.target_distance or 0) for skill in quality.per_skill),
        "featureOccurrences": {skill.code: skill.occurrences for skill in quality.per_skill},
...
def _compare_outcomes(
    ...
    score_key = "profileFitScore" if comparison_confidence == "FULL" else "surfaceProfileFitScore"
    risk_key = "riskPer10" if comparison_confidence == "FULL" else "surfaceRiskPer10"
    ...
    if comparable:
        ...
        if comparison_confidence == "FULL":
            comparison_reason = (
                "두 장을 동일한 아동 프로필과 Kiwi·G2P 기준으로 비교했습니다." + budget_note
            )
        else:
            comparison_reason = (
                "두 장 모두 점수를 표시하되, G2P가 확정하지 못한 "
                "PHONO 규칙은 제외하고 확인 가능한 규칙만 비교한 "
                "참고 결과입니다. 미검증 규칙을 0회로 판정하지 않습니다." + budget_note
            )
    else:
        score_delta = None
        winner = "UNVERIFIED"
        comparison_reason = (
            ...
            "아니어서 점수를 직접 비교하지 않았습니다." + budget_note
        )
    return {
        "comparable": comparable,
        "comparisonConfidence": comparison_confidence,
        "scoreBasis": ("FULL_POLICY" if comparison_confidence == "FULL" else "COMMON_SURFACE_ONLY"),
        "plainProfileFitScore": plain_score,
        "personalizedProfileFitScore": personalized_score,
        "winner": winner,
        "comparisonReason": comparison_reason,
        "delta": {
            "profileFitScore": (round(score_delta, 3) if score_delta is not None else None),
            "riskPer10": round(
                float(personalized_fit[risk_key]) - float(plain_fit[risk_key]),
                6,
            ),
            "excludedOverage": (
                int(personalized_fit["excludedOverage"]) - int(plain_fit["excludedOverage"])
            ),
            "limitedOverage": (
                int(personalized_fit["limitedOverage"]) - int(plain_fit["limitedOverage"])
            ),
            "targetDistance": (
                int(personalized_fit["targetDistance"]) - int(plain_fit["targetDistance"])
            ),
            "totalElapsedMs": round(
                float(personalized_timing["total"]) - float(plain_timing["total"]),
                3,
            ),
            "apiCallCount": (
                int(personalized_generation["apiCallCount"]) - int(plain_generation["apiCallCount"])
            ),
        },
    }
