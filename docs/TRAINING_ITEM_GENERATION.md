...
때 사용하는 발음 대상이다. 따라서 따라 보기 문항에서는 항상 `target`과 같은 값만
사용하며 `ㅁ을 따라 써요` 같은 안내 문구를 섞지 않는다. 같은 글자를 반복해서
연습하는 것은 허용한다. `traceAssetKey`도 프론트에 실제로 존재하는 획순 에셋만 사용한다.
...
제외 특성을 이용해 요청의 `count`와 같은 수의 후보를 반환한다. 현재 제품 백엔드는
훈련 하나당 3문항을 구성하므로 `count: 3`을 전달한다. 개발용 후보 비교에서는 최대
9개까지 요청할 수 있다.
