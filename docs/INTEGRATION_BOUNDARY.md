...
AI 서버는 기존 Backend가 호출하는 `/api/v1/story/generate`와
`/api/v1/story/continue`를 v3 개인화 생성기에 연결합니다. 첫 응답은 일반
대사 4개와 분기 질문 1개, 이어쓰기 응답은 일반 대사 5개로 변환되며 기존
Backend가 그대로 DB에 저장할 수 있습니다. `STORY_PROVIDER=gms` 또는
`openai`에서 실제 모델을 사용하고 `mock`에서는 결정적 mock을 사용합니다.
후보 선택 뒤 확정 위반 문장에는 조건부 국소 교정을 적용하되, 교정 실패나
품질 저하가 전체 이야기 생성을 막지 않고 원본 후보로 폴백합니다.

다만 현재 Backend v1 요청은 `studentId`만 보내고 실제 읽기 프로필을 보내지
않습니다. 따라서 호환 어댑터는 제한적인 임시 균형형 프로필을 사용합니다.
이 경로는 이야기 나라에 실제 생성 글을 표시하기 위한 단계적 통합이며,
아동별 개인화를 완료하려면 Backend가 컴파일한 `generationProfile`을 보내는
공식 v3 계약 연동이 필요합니다.

기존 Backend용 `/api/v1/images/generate`는 짧은 mock SVG URL만 반환합니다.
`STORY_IMAGE_PROVIDER=disabled` 상태에서는 실제 이미지 공급자를 호출하지
않으므로 글 통합 테스트에서 이미지 크레딧을 사용하지 않습니다.
...
- 기존 훈련 및 legacy 이미지 mock 경로 일부는 런타임 인증이 없습니다.
  v1 이야기 경로에는 내부 API 키 인증이 적용되어 있습니다. 나머지 경로도
